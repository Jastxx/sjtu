    ...
    function valQian() {
        ...
        $.each(course, function (i, kc) {
            ...
            $.get(url_kaoqin + "courseid=" + kc.id + "&term_identify=" + term, function (d) {
                var x = delCode(d);
                $.each($(x).find('table:eq(1) tr'),function(j,tr){
                    if(j==0)    return;
                    var dk = {};
                    kc.dk.push(dk);
                    dk = {};
                    dk.id = tr.find('td:eq(0)').text();//课程次数
                    dk.qDay = tr.find('td:eq(1)').text();//课程日期
                    dk.downstatus = tr.html().indexOf('hook') > -1;//已下载
                    dk.comUrl = tr.find('a')[0].href;//点评页url
                    var isCom = tr.html().indexOf('已填写') > -1;//已填写
                    var vTime = new Date(kc.dk.qDay).getTime() + weekTime;
                    var isExpire = !isCom && vTime < new Date().getTime();//已过期
                    dk.qianUse = !isCom && vTime > new Date().getTime();//是否可签

                    appendInfo("<br/><label style='color:" + (dk.qianUse ? "red" : '') + "'>《" + dk.name + "》：" + dk.qDay + " - " + (dk.downstatus ? '已签到' : '未签到') + " - " + (isCom?'已':'未') + "评价" + (isExpire ? '（已过期）' : '') + '</label>');
                    if (dk.qianUse)
                        comNum++;
...
//评价
    ...
    function pingjia() {
        runSS = true;
        $.each(course, function (i, kc) {
            $.each(kc,function(j,dk){
                if (dk.qianUse) {
                    //	++comNum;
                    var url_pingjia = 'http://218.1.73.12/PingJia/Default.aspx?sid=' + userid + '&cid=' + kc.id + '&ct=' + dk.qDay + '&term=' + term;
                    $.get(url_pingjia, function (d) {
                        var pid='pj_' + kc.id+'_'+dk.id;
                        if ($('#'+pid).length == 0)
                            appendInfo('《' + kc.name + '》：<label id="' + pid + '"></label><label id="wait_' + kc.id + '"></label>');
                        if (d.indexOf('考勤') > -1) {
                            $('#pj_' + kc.id).css('color', 'green').text('可评价');
                            $('#wait_' + kc.id).text("评价中...");
                            pjSubmit(d, url_pingjia, kc.id);
                            kc.qianUse = false;
                        } else {
                            $('#pj_' + kc.id).css('color', 'red').text('未下载');
                            $('#wait_' + kc.id).html("（<label class='ss'>10</label>秒后刷新）");
                        }
                    }, 'html');
                }
            });
